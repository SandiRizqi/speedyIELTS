    ...
    <div className="bg-gray-100 p-3 sm:p-4 rounded-xl shadow-lg max-w-full mx-auto">
      ...
      <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3 overflow-x-auto pb-3">
        ...
        <ScoreCategory 
          ...
          score={result?.cohen.score}
          items={result?.cohen.details.map((obj) => {return { score: obj.score, text: obj.text }})}
        />
        <ScoreCategory 
          title="TASK RESPONSE" 
          score={result?.task.score}
          items={result?.task.details.map((obj) => {return { score: obj.score, text: obj.text }})}
        />
      </div>
