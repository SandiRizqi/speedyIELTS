...
const getRate = (data: TestItem[]): number => {
  if (data.length === 0) return 0; // Handle empty array case

  const firstValue = data[0].overall; // Get the first overall value
  const total = data.reduce((sum, item) => sum + item.overall, 0); // Sum all overall values
  const average = total / data.length; // Calculate the average
  const selisih = average-firstValue;
  const rate = (selisih / firstValue) * 100; // Convert to percentage
  return parseFloat(rate.toFixed(2))// Format to 2 decimal places and add %
};
...
const MyDashboard: React.FC = () => {
  ...
  const getChartData = async () => {
    ...
    await getData({ id: userState.uid }).then((result: any) => {
      ...
      data['rates'] = {};
      data['rates'].listening = getRate(data['listening']);
      data['rates'].reading = getRate(data['reading']);
      data['rates'].speaking = getRate(data['speaking']);
      data['rates'].writing = getRate(data['writing']);
      setChartData(data);
      // console.log(data)
    ...
    <>
      {chartData ? (
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
          <CardDataStats title="Highest Listening" total={getHighest(chartData['listening'])} rate={chartData['rates']['listening'] + ' %'} levelUp={chartData['rates']['listening'] > 0 ? true: false} levelDown={chartData['rates']['listening'] <= 0 ? true: false}>
          ...
          <CardDataStats title="Highest Reading" total={getHighest(chartData['reading'])} rate={chartData['rates']['reading'] + ' %'} levelUp={chartData['rates']['reading'] > 0 ? true: false} levelDown={chartData['rates']['reading'] <= 0 ? true: false}>
            <svg
          ...
          <CardDataStats title="Highest Writing" total={getHighest(chartData['writing'])} rate={chartData['rates']['writing'] + ' %'} levelUp={chartData['rates']['writing'] > 0 ? true: false} levelDown={chartData['rates']['writing'] <= 0 ? true: false}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-5 w-5 fill-blue-600 dark:fill-white"
              viewBox="0 0 24 24"
          ...
          <CardDataStats title="Highest Speaking" total={getHighest(chartData['speaking'])} rate={chartData['rates']['speaking'] + ' %'} levelUp={chartData['rates']['speaking'] > 0 ? true: false} levelDown={chartData['rates']['speaking'] <= 0 ? true: false}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="#ffffff" viewBox="0 0 24 24" width="24px" height="24px" className="fill-blue-600 dark:fill-white">
